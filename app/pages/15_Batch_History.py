...
styled = (
    ...
    .style
    .applymap(_colour_pass, subset=["pass"] if "pass" in present_cols else [])
    .applymap(
        _colour_score,
        subset=[c for c in ["overall_score"] + DIMS if c in present_cols],
    )
    .format(
        {c: "{:.1f}" for c in ["overall_score"] + DIMS if c in present_cols},
        na_rep="N/A",
    )
    .set_properties(**{
